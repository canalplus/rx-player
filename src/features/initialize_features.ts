...
export default function initializeFeaturesObject(): void {
  ...
  if (HAS_MEDIA_SOURCE) {
    features.mediaSourceInit =
      require("../core/init/media_source_content_initializer.ts").default;
  }
  ...
  if ((__FEATURES__.DIRECTFILE as number) === (__FEATURES__.IS_ENABLED as number)) {
    ...
    const mediaElementTrackChoiceManager =
      require("../core/api/tracks_management/media_element_track_choice_manager.ts").default;
    features.directfile = { initDirectFile, mediaElementTrackChoiceManager };
  }
}
