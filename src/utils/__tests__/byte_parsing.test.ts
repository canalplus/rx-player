...
describe("utils - byte parsing", () => {
  ...
  describe("bytesToStr", () => {
    it("should return an empty string for an empty typedArray", () => {
      expect(byteUtils.bytesToStr(new Uint8Array(0))).to.equal("");
    });
    ...
    it("should return the right string for any typedArray", () => {
      ...
      expect(byteUtils.bytesToStr(arr8)).to.equal(
        someLetters
          .split("")
  ...
  describe("bytesToUTF16Str", () => {
    it("should return an empty string for an empty typedArray", () => {
      expect(byteUtils.bytesToUTF16Str(new Uint8Array([]))).to.equal("");
    });
    ...
    it("should consider every other byte for any typedArray", () => {
      ...
      expect(byteUtils.bytesToUTF16Str(arr8)).to.equal(
        skipOddLetters
          .split("")
          .map(l => String.fromCharCode(l.charCodeAt(0) & 0xFF))
          .join("")
  ...
  describe("bytesToHex", () => {
    it("should return an empty string for an empty typedArray", () => {
      expect(byteUtils.bytesToHex(new Uint8Array([]))).to.equal("");
    });

    it("should convert to hexadecimal Uint8Array instances", () => {
      const arr = new Uint8Array([255, 9, 254, 2]);
      expect(byteUtils.bytesToHex(arr)).to.equal("ff09fe02");
    });

    it("should allow to add a separator", () => {
      const arr = new Uint8Array([255, 9, 254, 2]);
      expect(byteUtils.bytesToHex(arr, "--")).to.equal("ff--09--fe--02");
    });
  });
  ...
  describe("be2toi", () => {
    it("should return 0 for an empty TypedArray", () => {
      expect(byteUtils.be2toi(new Uint8Array(0), 0)).to.equal(0);
    });

    it("should return 0 for an out-of-bound offset", () => {
      const arr = new Uint8Array([255, 255, 1, 8]);
      expect(byteUtils.be2toi(arr, -45)).to.equal(0);
      expect(byteUtils.be2toi(arr, 45)).to.equal(0);
    });
    ...
    /* tslint:enable:max-line-length */
      ...
      expect(byteUtils.be2toi(arr, 0)).to.equal(expected[0]);
  ...
  describe("be4toi", () => {
    ...
    /* tslint:enable:max-line-length */
      ...
      expect(byteUtils.be4toi(arr, 0)).to.equal(expected[0]);
      expect(byteUtils.be4toi(arr, 1)).to.equal(expected[1]);
      expect(byteUtils.be4toi(arr, 2)).to.equal(expected[2]);
      expect(byteUtils.be4toi(arr, 3)).to.equal(expected[3]);
    });
  });
  ...
  describe("be8toi", () => {
    ...
    /* tslint:enable:max-line-length */
      ...
      expect(byteUtils.be8toi(arr, 0)).to.equal(expected[0]);
      expect(byteUtils.be8toi(arr, 8)).to.equal(expected[1]);
    });
  });

  describe("le2toi", () => {
    it("should return 0 for an empty TypedArray", () => {
      expect(byteUtils.le2toi(new Uint8Array(0), 0)).to.equal(0);
    });

    it("should return 0 for an out-of-bound offset", () => {
      const arr = new Uint8Array([255, 255, 1, 8]);
      expect(byteUtils.le2toi(arr, -45)).to.equal(0);
      expect(byteUtils.le2toi(arr, 45)).to.equal(0);
    });
    ...
    it(
      ...
      () => {
        ...
        expect(byteUtils.le2toi(arr, 0)).to.equal(expected[0]);
        expect(byteUtils.le2toi(arr, 1)).to.equal(expected[1]);
        expect(byteUtils.le2toi(arr, 2)).to.equal(expected[2]);
        expect(byteUtils.le2toi(arr, 3)).to.equal(expected[3]);
      });
  });
  ...
  describe("le4toi", () => {
    it(
      ...
      () => {
        ...
        expect(byteUtils.le4toi(arr, 0)).to.equal(expected[0]);
        expect(byteUtils.le4toi(arr, 1)).to.equal(expected[1]);
        expect(byteUtils.le4toi(arr, 2)).to.equal(expected[2]);
        expect(byteUtils.le4toi(arr, 3)).to.equal(expected[3]);
      });
  });
      ...
      () => {
        ...
        const arr = new Uint8Array([
          ...
          0, 255, 0, 255, 0, 255, 0, 0 ]); // I can't set the top-most byte or I
                                           // go over MAX_SAFE_INTEGER
        const expected = [ 1, 280379743338240 ];
        expect(byteUtils.le8toi(arr, 0)).to.equal(expected[0]);
        expect(byteUtils.le8toi(arr, 8)).to.equal(expected[1]);
      });
  });
  ...
  describe("itole2", () => {
    ...
    /* tslint:enable:max-line-length */
      ...
      expect(byteUtils.itole2(values[0])).to.deep.equal(new Uint8Array([8, 1]));
      expect(byteUtils.itole2(values[1])).to.deep.equal(new Uint8Array([1, 255]));
      expect(byteUtils.itole2(values[2])).to.deep.equal(new Uint8Array([255, 255]));
    });
  });
