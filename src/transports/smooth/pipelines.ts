...
import log from "../../log";
import type { Adaptation, ISegment } from "../../manifest";
...
import type { INextSegmentsInfos } from "./extract_timings_infos";
...
function addNextSegments(
  ...
  for (let i = 0; i < representations.length; i++) {
    ...
    if (
      representation.index instanceof SmoothRepresentationIndex &&
      dlSegment?.privateInfos?.smoothMediaSegment !== undefined
    ) {
      representation.index.addNewSegments(
        nextSegments,
...
export default function (transportOptions: ITransportOptions): ITransportPipelines {
  ...
  const manifestPipeline = {
    ...
    resolveManifestUrl(
      ...
      if (WSX_REG.test(url)) {
        warnOnce(
          "Giving WSX URL to loadVideo is deprecated." +
  ...
  const textTrackPipeline = {
    ...
    > {
      ...
      if (!isMP4) {
        ...
        }).then((data) => ({ resultType: "segment-loaded" as const, resultData: data }));
      } else {
        return request({
          url,
  ...
  const imageTrackPipeline = {
    ...
    > {
      ...
      if (segment.isInit || url === null) {
        ...
        return { resultType: "segment-created" as const, resultData: null };
      }
