...
export default class PlaybackObserver {
  ...
  constructor(mediaElement: IMediaElement | null, options: IPlaybackObserverOptions) {
    ...
    this._mediaElementRef = new SharedReference(mediaElement);
  ...
  onMediaElementAttachment(
    ...
    if (mediaElement !== null) {
      try {
        ...
        log.error(
          "API: onMediaElementAttachment error",
  ...
  /**
   * A `MediaElementPlaybackObserver` may havbeen e initialized without an
   * `HTMLMediaElement` first as content was not actually playing on an
   * `HTMLMediaElement initially (e.g. it was only "pre-loading" in memory).
   *
   * In this case, calling `attachMediaElement` allows to "link" the
   * `HTMLMediaElement` when it is finally intended to play it.
   *
  ...
  public getCurrentTime(): number | undefined {
    return this._mediaElementRef.getValue()?.currentTime;
  }
  ...
  public getPlaybackRate(): number | undefined {
    return this._mediaElementRef.getValue()?.playbackRate;
  }
  ...
  public getIsPaused(): boolean | undefined {
    return this._mediaElementRef.getValue()?.paused;
  }
  ...
  public setCurrentTime(time: number): void {
    const mediaElement = this._mediaElementRef.getValue();
    if (mediaElement !== null && mediaElement.readyState >= 1) {
      this._actuallySetCurrentTime(mediaElement, time);
    } else {
      this._internalSeeksIncoming = [];
  ...
  public getReadyState(): number | undefined {
    return this._mediaElementRef.getValue()?.readyState;
  }
  ...
  private _onLoadedMetadataEvent(): void {
    const mediaElement = this._mediaElementRef.getValue();
    if (mediaElement !== null && this._pendingSeek !== null) {
