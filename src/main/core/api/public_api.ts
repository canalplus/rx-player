...
import EventEmitter, {
  IListener,
} from "../../../common/utils/event_emitter";
...
import {
  canRelyOnVideoVisibilityAndSize,
  events,
  exitFullscreen,
  getStartDate,
  isFullscreen,
...
import { InitializeOnMediaSource } from "../init/worker_init";
...
class Player extends EventEmitter<IPublicAPIEvent> {
  ...
  private readonly _priv_worker : Worker;
  ...
  private _priv_contentInfos : null | {
     ...
     */
    activeAdaptations : {
      [periodId : string] : Partial<Record<IBufferType, ISentAdaptation|null>>;
    } | null;
    ...
    activeRepresentations : {
      [periodId : string] : Partial<Record<IBufferType, ISentRepresentation|null>>;
    } | null;
  ...
  ) : Partial<Record<IBufferType, ISentAdaptation|null>> | null {
  ...
  ) : Partial<Record<IBufferType, ISentRepresentation|null>> | null {
   ...
   * @returns {Array.<Object>|null}
   */
  __priv_getSegmentBufferContent() : null {
    return null;
  }
   ...
   * @param {Object} value
   */
  private _priv_onManifestReady({ manifest } : { manifest : ISentManifest }) : void {
   ...
   * @param {Object} value
   */
  private _priv_onActivePeriodChanged({ period } : { period : ISentPeriod }) : void {
    if (this._priv_contentInfos === null) {
  ...
  private _priv_onAdaptationChange({
    ...
    type : IBufferType;
    adaptation : ISentAdaptation|null;
    period : ISentPeriod;
  }) : void {
    if (this._priv_contentInfos === null) {
  ...
  private _priv_onRepresentationChange({
    ...
    type : IBufferType;
    period : ISentPeriod;
    representation : ISentRepresentation|null;
  }) : void {
    if (this._priv_contentInfos === null) {
 ...
 * @returns {Array.<Number>}
 */
function getAvailableBitrates(adaptation : ISentAdaptation) : number[] {
