(() => {
  ...
  var Observable = function() {
    function Observable24(subscribe) {
      if (subscribe) {
        this._subscribe = subscribe;
      }
    }
    Observable24.prototype.lift = function(operator) {
      var observable2 = new Observable24();
      observable2.source = this;
      observable2.operator = operator;
      return observable2;
    };
    Observable24.prototype.subscribe = function(observerOrNext, error, complete) {
      var _this = this;
    ...
    Observable24.prototype._trySubscribe = function(sink) {
      try {
    ...
    Observable24.prototype.forEach = function(next, promiseCtor) {
      var _this = this;
    ...
    Observable24.prototype._subscribe = function(subscriber) {
      var _a;
      return (_a = this.source) === null || _a === void 0 ? void 0 : _a.subscribe(subscriber);
    };
    Observable24.prototype[observable] = function() {
      return this;
    };
    Observable24.prototype.pipe = function() {
      var operations = [];
      for (var _i = 0; _i < arguments.length; _i++) {
    ...
    Observable24.prototype.toPromise = function(promiseCtor) {
      var _this = this;
      promiseCtor = getPromiseCtor(promiseCtor);
      return new promiseCtor(function(resolve, reject) {
        var value;
    ...
    Observable24.create = function(subscribe) {
      return new Observable24(subscribe);
    };
    return Observable24;
  }();
    ...
    const estimate$ = defer(() => {
      ...
      return manualBitrate$.pipe(switchMap((manualBitrate) => {
        ...
        return combineLatest([
          ...
          const { bandwidthEstimate, bitrateChosen } = networkAnalyzer.getBandwidthEstimate(observation, bandwidthEstimator, currentRepresentation, requests, prevEstimate.bandwidth);
  ...
  var ABRManager = class {
    ...
    get$(context2, representations, observation$, streamEvents$) {
      ...
      const manualBitrate$ = takeFirstSet(this._manualBitrates[type]?.asObservable(), of(-1));
      const minAutoBitrate$ = takeFirstSet(this._minAutoBitrates[type]?.asObservable(), of(0));
      const maxAutoBitrate$ = takeFirstSet(this._maxAutoBitrates[type]?.asObservable(), of(Infinity));
    ...
    _createInnerObservable() {
      return defer(() => {
        ...
        const getCurrentObservation = (event) => {
          ...
          const lastObservation = this._lastObservation ?? this._generateInitialObservation();
        ...
        return merge(interval$).pipe(map((event) => {
...
${event}`);
    ...
    _generateInitialObservation() {
      return object_assign_default(getMediaInfos("init"), {
        rebuffering: null,
        freezing: null,
  ...
  function createSegmentFetcher(bufferType, pipeline, callbacks, options) {
    ...
    return function fetchSegment(content) {
      ...
      return new Observable((obs) => {
        ...
        tryURLsWithBackoff(segment.mediaURLs ?? [null], callLoaderWithUrl, object_assign_default({ onRetry }, options), canceller.signal).then((res) => {
...
[${letterInfo.letter}] P: ${letterInfo.periodId} || R: ${letterInfo.representationId}(${letterInfo.bitrate ?? "unknown bitrate"})`;
  ...
  var initManagers = (mediaSource, mpdUrl) => {
    ...
    const segmentBufferStore = new segment_buffers_default(mediaSource);
    const dashPipeline = dash_default({ lowLatencyMode: false });
    const segmentFetcherCreator = new segment_default(dashPipeline, {
      lowLatencyMode: false,
      maxRetryOffline: void 0,
      maxRetryRegular: void 0
    });
    const manifestFetcher = new manifest_default2(mpdUrl, dashPipeline, {
      lowLatencyMode: false,
      maxRetryOffline: void 0,
      maxRetryRegular: void 0
    });
        ...
        ev.parse({
          ...
          if (parseEvent.type === "parsed") {
            ...
            const streamPlaybackObserver = createStreamPlaybackObserver(manifest, playbackObserver, {
              ...
              initialSeekPerformed: reference_default(false),
            ...
            const stream = stream_default({ initialPeriod: manifest.periods[0], manifest }, streamPlaybackObserver, abrManager, segmentBufferStore, segmentFetcherCreator, {
              ...
              maxBufferAhead: reference_default(Infinity),
              maxBufferBehind: reference_default(Infinity),
