...
function createCompatModule({ MediaSource, HTMLVideoElement, URL }) {
  ...
  HTMLVideoElement = HTMLVideoElement || win.HTMLVideoElement;
  ...
  var isIE = (
    ...
    nav.appName == "Netscape" && /Trident\//.test(nav.userAgent)
  );
  ...
  if (nav.requestMediaKeySystemAccess)
    requestMediaKeySystemAccess = (a, b) => nav.requestMediaKeySystemAccess(a, b);
  ...
  function isEventSupported(element, eventNameSuffix) {
    var clone = doc.createElement(element.tagName);
  ...
  function isCodecSupported(codec) {
    return !!MediaSource && MediaSource.isTypeSupported(codec);
  }
  ...
  if (!requestMediaKeySystemAccess && HTMLVideoElement.prototype.webkitGenerateKeyRequest) {
  ...
  function requestFullscreen(elt) {
    ...
    if (elt.webkitRequestFullscreen) return elt.webkitRequestFullscreen(win.Element.ALLOW_KEYBOARD_INPUT);
  }
