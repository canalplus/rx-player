...
type IVendorGlobalScope = typeof globalScope & {
  ...
  webOS?:
    | {
        deviceInfo:
          ((cb: (arg: IWebOsDeviceCallback) => void) => void) | null | undefined;
      }
    | null
    | undefined;
};
...
export default function getMaxSupportedResolution(): {
  ...
  try {
    if (EnvDetector.device === EnvDetector.DEVICES.Hisense) {
      if (
    ...
    if (EnvDetector.device === EnvDetector.DEVICES.Tizen) {
      if (
    ...
    log.error(
      "utils",
      "Error when trying to call vendor API",
