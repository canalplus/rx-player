...
class DummySourceBufferListBase extends EventEmitter<ISourceBufferListEventMap> {
  public onaddsourcebuffer: ((evt: Event) => void) | null;
  public onremovesourcebuffer: ((evt: Event) => void) | null;
  constructor() {
    super();
...
interface ISourceBufferArray extends Array<ISourceBuffer> {
  [index: number]: ISourceBuffer;
}

type IDummyMediaSourceSourceBufferList = ISourceBuffer[] & {
  onaddsourcebuffer: ((evt: Event) => void) | null;
  onremovesourcebuffer: ((evt: Event) => void) | null;
} & IEventTarget<ISourceBufferListEventMap>;
  ...
  implements IMediaSource
{
  ...
  public sourceBuffers: IDummyMediaSourceSourceBufferList;
  ...
  constructor() {
    ...
    const sourceBuffers: ISourceBuffer[] = [];
    const base = new DummySourceBufferListBase();
    const sourceBufferList = objectAssign(sourceBuffers as ISourceBufferArray, base);

    // I don't understand why TypeScript is not happy here
    this.sourceBuffers = sourceBufferList as IDummyMediaSourceSourceBufferList;
  ...
  public addSourceBuffer(givenType: string): ISourceBuffer {
    ...
    this.eventScheduler.schedule(this.sourceBuffers, "addsourcebuffer", null).catch(noop);
    return sb;
  }
  ...
  public removeSourceBuffer(sb: ISourceBuffer): void {
    ...
    if (index >= 0) {
      ...
      this.eventScheduler
        .schedule(this.sourceBuffers, "removesourcebuffer", null)
        .catch(noop);
    }
  }
  ...
  public appendBuffer(data: BufferSource): void {
    ...
    this.eventScheduler
      ...
      .then(() => {
        ...
        const chunks = splitCompleteChunks(u8data);
        if (chunks[0] === null) {
          this.updating = false;
...
interface IDummyMediaElementEventObject {
  resolve: () => void;
  reject: (e: unknown) => void;
...
interface IDummySourceBufferListEventObject {
  resolve: () => void;
  reject: (e: unknown) => void;
  obj: IDummyMediaSourceSourceBufferList;
  evtName: keyof ISourceBufferListEventMap;
  cancelSignal: CancellationSignal | null;
}
export class EventScheduler {
  private _scheduled: Array<
    | IDummySourceBufferEventObject
    | IDummyMediaSourceEventObject
    | IDummyMediaElementEventObject
    | IDummySourceBufferListEventObject
  >;
  constructor() {
    this._scheduled = [];
  }
  ...
  public schedule(
    obj:
      | DummyMediaElement
      | IDummyMediaSourceSourceBufferList
      | DummyMediaSource
      | DummySourceBuffer,
    evtName:
      | keyof IMediaElementEventMap
      | keyof ISourceBufferListEventMap
