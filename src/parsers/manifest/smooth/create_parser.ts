...
function createSmoothStreamingParser(
...
): (manifest: Document, url?: string, manifestReceivedTime?: number) => IParsedManifest {
  ...
  function parseAdaptation(args: IAdaptationParserArguments): IParsedAdaptation | null {
    ...
    const index = { timeline: parseCNodes(cNodes), timescale: _timescale };
    ...
    const representations = qualityLevels.map((qualityLevel) => {
      const repIndex = {
        ...
        media: replaceRepresentationSmoothTokens(
          UrlPathWithTokens,
      ...
      const aggressiveMode =
        parserOptions.aggressiveMode == null
          ? DEFAULT_AGGRESSIVE_MODE
          : parserOptions.aggressiveMode;
      const reprIndex = new RepresentationIndex(repIndex, {
        aggressiveMode,
        isLive,
        manifestReceivedTime,
        segmentPrivateInfos,
  ...
  function parseFromDocument(
    ...
    const manifest = {
      ...
      timeBounds: { minimumSafePosition: minimumTime, timeshiftDepth, maximumTimeData },
      periods: [
        {
          adaptations,
