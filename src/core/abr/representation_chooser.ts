...
export default class RepresentationChooser {
  ...
  public get$(
    ...
    const filtersArray$ : Array<Observable<IFilters>> = [];

    if (this._limitWidth$) {
      filtersArray$.push(this._limitWidth$.map(width => ({ width })));
    }

    filtersArray$.push(
      Observable.combineLatest(
        maxDecodableBitrate$.startWith(Infinity),
        this._throttle$ ? this._throttle$ : Observable.of(Infinity)
      )
      .map((args) => {
    ...
    const filters$ = filtersArray$.length ?
      Observable.combineLatest(...filtersArray$)
