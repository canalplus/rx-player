...
- A `MediaElementMonitor` emitting the current time (in seconds) when the garbage
...
Basically, each times the given `MediaElementMonitor` emits, the BufferGarbageCollector
will ensure that the volume of data before and ahead of the current position does not grow
into a larger value than what is configured.
