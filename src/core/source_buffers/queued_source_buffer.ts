...
// Informations to give when pushing a new chunk via the `pushChunk` method.
export interface IPushChunkInfos<T> { data : IPushedChunkData<T>;
                                      inventoryInfos : IPushedChunkInventoryInfos; }

// Informations to give when indicating a whole segment has been pushed via the
// `endOfSegment` method.
...
export default class QueuedSourceBuffer<T> {
  ...
  // Informations about the current action processed by the QueuedSourceBuffer.
          ...
          this._pendingTask.steps.length === 0)
      {
        switch (this._pendingTask.type) {
          case SourceBufferAction.Push:
            this._segmentInventory.insertChunk(this._pendingTask.inventoryInfos);
            break;
