...
export interface IGetNeededSegmentsArguments {
  ...
  segmentsBeingPushed : Array<ISegmentMixedWithContent>;
  /**
...
export default function getNeededSegments({
  ...
  const estimateSizePushed = segmentsBeingPushed.reduce((size, segment) => {
    const {bitrate} = segment.representation
    // Not taking into account the fact that i can still be generated and the duration not fully exact
    const {duration} = segment.segment
    return size + (bitrate*duration)/1000
  }, 0)
  availableBufferSize -= estimateSizePushed
  ...
  return availableSegmentsForRange.filter(segment => {
    ...
    // being pushed.
    if (isSegmentPushedInOtherRepr(segmentsBeingPushed, contentObject, currentPlaybackTime, fastSwitchThreshold))
...
const isSegmentAlreadyDownloaded = (segment: ISegment, segmentsToKeep: IBufferedChunk[], content: IContentContext) => {
...
const isSegmentBeingPushed = (segmentsBeingPushed: ISegmentMixedWithContent[], contentObject: ISegmentMixedWithContent) => {
...
const isSegmentPushedInOtherRepr = (segmentsBeingPushed: ISegmentMixedWithContent[], contentObject: ISegmentMixedWithContent,  currentPlaybackTime: number, fastSwitchThreshold?: number) => {
  // Check if the same segment from another Representation is not already
    // being pushed.
