...
describe("ABR - filterByWidth", () => {
  const fakeReps = [{ width: 100 }, { width: 1000 }, { width: 10000 }, { width: 100000 }];

  const expectedWidthReps = [{ width: 100 }, { width: 1000 }];

  describe("filterByWidth", () => {
    it("should properly filter representations whose width is < 900", () => {
      expect(filterByWidth(fakeReps as Representation[], 999)).toEqual(expectedWidthReps);
    });

    it("should return all representations when specified width is over maxWidth", () => {
      expect(filterByWidth(fakeReps as Representation[], 1000000)).toEqual(fakeReps);
    });

    it("should return all representations when specified width is infinite", () => {
      expect(filterByWidth(fakeReps as Representation[], Infinity)).toEqual(fakeReps);
    });

    it("should return first representation when specified width is 0", () => {
      expect(filterByWidth(fakeReps as Representation[], 0)).toEqual([{ width: 100 }]);
    });

    it("should return no representation when specified array is empty", () => {
      expect(filterByWidth([], 0)).toEqual([]);
    });
  });
});
