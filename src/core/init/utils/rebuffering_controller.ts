...
import type { IPlaybackObservation, PlaybackObserver } from "../../api";
...
export default class RebufferingController extends EventEmitter<IRebufferingControllerEvent> {
  ...
  public start(): void {
    ...
    this._playbackObserver.listen(
      (observation) => {
        ...
        if (rebuffering === null) {
          ...
          if (readyState === 1) {
            ...
            if (observation.seeking) {
              reason =
                observation.pendingInternalSeek !== null ? "internal-seek" : "seeking";
            } else {
              reason = "not-ready";
            }
        ...
        const stalledReason =
          rebuffering.reason === "seeking" && observation.pendingInternalSeek !== null
        ...
        if (ignoredStallTimeStamp !== null) {
          ...
          if (now - ignoredStallTimeStamp < FORCE_DISCONTINUITY_SEEK_DELAY) {
            ...
            log.warn(
              "Init: ignored stall for too long, considering it",
   ...
   */
  private _checkDecipherabilityFreeze(observation: IPlaybackObservation): boolean {
