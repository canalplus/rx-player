...
// single preference for an video track Adaptation
...
export default class VideoTrackManager {
  ...
  public getChosenVideoTrack(period : Period) : ILMVideoTrack|null {
    ...
    return {
      role: chosenVideoAdaptation.role,
...
function findFirstOptimalVideoAdaptation(
  ...
  if (!videoAdaptations.length) {
    return null;
  }

  const mainAdaptation = arrayFind(videoAdaptations, (videoAdaptation) =>
    videoAdaptation.role === "main"
  );

  // Prefer main adaptation to first video adaptation
  return mainAdaptation || videoAdaptations[0];
}
