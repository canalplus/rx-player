...
function displayWebpackErrors(errors, warnings) {
  ...
  for (let i = 0; i < warnings.length; i++) {
    ...
    console.error(`\x1b[33mWarning from ${warning.loaderSource}`);
    console.error(
      `\x1b[36m${warning.file}:` +
      `\x1b[35m${warning.location.line}:${warning.location.character}`
    );
    console.error(warning.message);
  }
  for (let i = 0; i < errors.length; i++) {
    const error = errors[i];
    console.error(`\x1b[31mError from ${error.loaderSource}`);
    console.error(
      `\x1b[36m${error.file}:` +
      `\x1b[35m${error.location.line}:${error.location.character}`
    );
