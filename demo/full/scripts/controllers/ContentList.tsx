...
const MediaKeys_ = win.MediaKeys ||
                   win.MozMediaKeys ||
                   win.WebKitMediaKeys ||
                   win.MSMediaKeys ||
                   null;
...
function formatContent(
  content: IStoredContentInfo | IDefaultContent
): IDisplayedContent {
  ...
  return {
    ...
    fallbackKeyError: "fallbackKeyError" in content &&
      content.fallbackKeyError,
    fallbackLicenseRequest: "fallbackLicenseRequest" in content &&
      content.fallbackLicenseRequest,
...
function constructContentList(): Partial<Record<string, IDisplayedContent[]>> {
  ...
  const storedAndRegularContents = [
    ...reversedStoredContents,
    ...DEFAULT_CONTENTS
  ];
  return TRANSPORT_TYPES.reduce((
    acc: Partial<Record<string, IDisplayedContent[]>>,
    tech: string
  ) => {
 ...
 * @returns {number}
 */
function getIndexOfFirstEnabledContent(
  contentList: IDisplayedContent[]
): number {
...
function ContentList({
  ...
  const [
    ...
    setShouldDisplayGeneratedLink
  ] = React.useState(false);
  const [
    shouldDisplayDRMSettings,
    setShouldDisplayDRMSettings
  ] = React.useState(false);
  const [
    shouldFallbackOnKeyError,
    setShouldFallbackOnKeyError,
  ] = React.useState(false);
  const [
    shouldFallbackOnLicenseReqError,
    setShouldFallbackOnLicenseReqError
  ] = React.useState(false);
   ...
   */
  const loadContent = React.useCallback((content: IDisplayedContent) => {
    const {
      url,
      transport,
      fallbackKeyError,
  ...
  React.useEffect(() => {
    ...
    if (parsedHash !== null) {
      ...
      if (typeof tech === "string" && TRANSPORT_TYPES.includes(tech)) {
        ...
        const chosenDRMType =
          typeof parsedHash.drm === "string" &&
          DRM_TYPES.includes(parsedHash.drm) ?
            parsedHash.drm :
            undefined;
  ...
  if (shouldDisplayGeneratedLink) {
    generatedLink = contentChoiceIndex === 0 || isSavingOrUpdating ?
      generateLinkForCustomContent({
        chosenDRMType: shouldDisplayDRMSettings ?
          chosenDRMType :
          undefined,
        customKeySystem: shouldDisplayDRMSettings ?
          customKeySystem :
          undefined,
        fallbackKeyError: shouldFallbackOnKeyError,
        fallbackLicenseRequest: shouldFallbackOnLicenseReqError,
        manifestURL: currentManifestURL,
        licenseServerUrl: shouldDisplayDRMSettings ?
          licenseServerUrl :
          undefined,
  ...
  const onServerCertificateInput = React.useCallback(
  ...
  const onChangeDisplayDRMSettings = React.useCallback(
  ...
  const onLowLatencyClick = React.useCallback(
    (value: boolean) => {
  ...
  const onDRMTypeClick = React.useCallback(
    (value: string) => {
  ...
  const onChangeFallbackLicenseRequest = React.useCallback(
    (should: boolean) => {
