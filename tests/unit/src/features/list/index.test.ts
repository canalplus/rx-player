import { describe, it, expect } from "vitest";
import { DASH as DASH_FEATURE } from "../../../../../src/features/list/dash.ts";
import { DASH_WASM as DASH_WASM_FEATURE } from "../../../../../src/features/list/dash_wasm.ts";
import { DEBUG_ELEMENT as DEBUG_ELEMENT_FEATURE } from "../../../../../src/features/list/debug_element.ts";
import { DIRECTFILE as DIRECTFILE_FEATURE } from "../../../../../src/features/list/directfile.ts";
import { EME as EME_FEATURE } from "../../../../../src/features/list/eme.ts";
import { HTML_SAMI_PARSER as HTML_SAMI_PARSER_FEATURE } from "../../../../../src/features/list/html_sami_parser.ts";
import { HTML_SRT_PARSER as HTML_SRT_PARSER_FEATURE } from "../../../../../src/features/list/html_srt_parser.ts";
import { HTML_TEXT_BUFFER as HTML_TEXT_BUFFER_FEATURE } from "../../../../../src/features/list/html_text_buffer.ts";
import { HTML_TTML_PARSER as HTML_TTML_PARSER_FEATURE } from "../../../../../src/features/list/html_ttml_parser.ts";
import { HTML_VTT_PARSER as HTML_VTT_PARSER_FEATURE } from "../../../../../src/features/list/html_vtt_parser.ts";
import {
  DASH,
  DASH_WASM,
  DIRECTFILE,
  DEBUG_ELEMENT,
  EME,
  HTML_SAMI_PARSER,
  HTML_SRT_PARSER,
  HTML_TEXT_BUFFER,
  HTML_TTML_PARSER,
  HTML_VTT_PARSER,
  MULTI_THREAD,
  NATIVE_SAMI_PARSER,
  NATIVE_SRT_PARSER,
  NATIVE_TEXT_BUFFER,
  NATIVE_TTML_PARSER,
  NATIVE_VTT_PARSER,
  SMOOTH,
} from "../../../../../src/features/list/index.ts";
import { MULTI_THREAD as MULTI_THREAD_FEATURE } from "../../../../../src/features/list/multi_thread.ts";
import { NATIVE_SAMI_PARSER as NATIVE_SAMI_PARSER_FEATURE } from "../../../../../src/features/list/native_sami_parser.ts";
import { NATIVE_SRT_PARSER as NATIVE_SRT_PARSER_FEATURE } from "../../../../../src/features/list/native_srt_parser.ts";
import { NATIVE_TEXT_BUFFER as NATIVE_TEXT_BUFFER_FEATURE } from "../../../../../src/features/list/native_text_buffer.ts";
import { NATIVE_TTML_PARSER as NATIVE_TTML_PARSER_FEATURE } from "../../../../../src/features/list/native_ttml_parser.ts";
import { NATIVE_VTT_PARSER as NATIVE_VTT_PARSER_FEATURE } from "../../../../../src/features/list/native_vtt_parser.ts";
import { SMOOTH as SMOOTH_FEATURE } from "../../../../../src/features/list/smooth.ts";

describe("Features list", () => {
  it("should export all stable features", () => {
    expect(DASH).toBe(DASH_FEATURE);
    expect(DASH_WASM).toBe(DASH_WASM_FEATURE);
    expect(DIRECTFILE).toBe(DIRECTFILE_FEATURE);
    expect(EME).toBe(EME_FEATURE);
    expect(HTML_SAMI_PARSER).toBe(HTML_SAMI_PARSER_FEATURE);
    expect(HTML_SRT_PARSER).toBe(HTML_SRT_PARSER_FEATURE);
    expect(HTML_TEXT_BUFFER).toBe(HTML_TEXT_BUFFER_FEATURE);
    expect(HTML_TTML_PARSER).toBe(HTML_TTML_PARSER_FEATURE);
    expect(HTML_VTT_PARSER).toBe(HTML_VTT_PARSER_FEATURE);
    expect(MULTI_THREAD).toBe(MULTI_THREAD_FEATURE);
    expect(NATIVE_SAMI_PARSER).toBe(NATIVE_SAMI_PARSER_FEATURE);
    expect(NATIVE_SRT_PARSER).toBe(NATIVE_SRT_PARSER_FEATURE);
    expect(NATIVE_TEXT_BUFFER).toBe(NATIVE_TEXT_BUFFER_FEATURE);
    expect(NATIVE_TTML_PARSER).toBe(NATIVE_TTML_PARSER_FEATURE);
    expect(NATIVE_VTT_PARSER).toBe(NATIVE_VTT_PARSER_FEATURE);
    expect(SMOOTH).toBe(SMOOTH_FEATURE);
    expect(DEBUG_ELEMENT).toBe(DEBUG_ELEMENT_FEATURE);
  });
});
