...
export default function launchTestsForContent(manifestInfos) {
  ...
  const videoRepresentationsForFirstPeriod =
    ...
    periodsInfos[firstPeriodIndex].adaptations.video.length
      ...
      : [];
  const videoBitrates = videoRepresentationsForFirstPeriod.map(
  ...
  const audioRepresentationsForFirstPeriod =
    ...
    periodsInfos[firstPeriodIndex].adaptations.audio.length
      ...
      : [];
  const audioBitrates = audioRepresentationsForFirstPeriod.map(
    (representation) => representation.bitrate,
  );
  ...
  describe("API tests", () => {
    ...
    describe("getManifest", () => {
      it("should return the manifest correctly parsed", async function () {
        ...
        for (let periodIndex = 0; periodIndex < periodsInfos.length; periodIndex++) {
          ...
          expect(period.end).to.equal(periodInfos.start + periodInfos.duration);
          ...
          Object.keys(allAdaptations).forEach((type) => {
            ...
            for (
              ...
              const bitrates = adaptationInfos.representations.map(
                (representation) => representation.bitrate,
              );

              expect(!!adaptation.isAudioDescription).to.equal(
                !!adaptationInfos.isAudioDescription,
              );

              expect(!!adaptation.isClosedCaption).to.equal(
                !!adaptationInfos.isClosedCaption,
              );

              expect(adaptation.language).to.equal(adaptationInfos.language);

              expect(adaptation.normalizedLanguage).to.equal(
                adaptationInfos.normalizedLanguage,
              );

              expect(adaptation.getAvailableBitrates()).to.eql(bitrates);

              expect(typeof adaptation.id).to.equal("string");
              expect(adaptation.representations.length).to.equal(
                adaptationInfos.representations.length,
              );
              ...
              for (
                ...
                const representationInfos =
                  adaptationInfos.representations[representationIndex];

                expect(representation.bitrate).to.equal(representationInfos.bitrate);

                expect(representation.codec).to.equal(representationInfos.codec);

                expect(typeof representation.id).to.equal("string");

                expect(representation.mimeType).to.equal(representation.mimeType);
                ...
                if (initSegmentInfos) {
                  expect(initSegment.url).to.equal(initSegmentInfos.url);
                ...
                if (reprIndexInfos.segments.length) {
                  ...
                  const firstSegmentTime = reprIndexInfos.segments[0].time / timescale;
                  const rangeDuration =
                    (reprIndexInfos.segments[0].time +
                      reprIndexInfos.segments[0].duration / 2) /
                      timescale -
                    firstSegmentTime;
                  const firstSegments = reprIndex.getSegments(
                    firstSegmentTime,
                    rangeDuration,
                  );
                  ...
                  expect(firstSegment.time).to.equal(reprIndexInfos.segments[0].time);

                  expect(firstSegment.timescale).to.equal(
                    reprIndexInfos.segments[0].timescale,
                  );

                  expect(firstSegment.url).to.equal(reprIndexInfos.segments[0].url);
                }
              }
            }
          });
    ...
    describe("getCurrentAdaptations", () => {
      it("should return the currently played adaptations", async function () {
        ...
        expect(currentAdaptations.video).to.equal(
        ...
        expect(currentAdaptations.image).to.equal(
          (player.getManifest().periods[firstPeriodIndex].adaptations.image &&
            player.getManifest().periods[firstPeriodIndex].adaptations.image[0]) ||
            null,
        );
        expect(currentAdaptations.audio).to.equal(
          player.getManifest().periods[firstPeriodIndex].adaptations.audio &&
            player.getManifest().periods[firstPeriodIndex].adaptations.audio[0],
        );
      });
    });
    ...
    describe("getCurrentRepresentations", () => {
      it("should return the currently played representations", async () => {
        ...
        expect(currentRepresentations.video).to.equal(
        ...
        expect(currentRepresentations.image).to.equal(
          (player.getCurrentAdaptations().image &&
            player.getCurrentAdaptations().image.representations[0]) ||
            undefined,
        );
        expect(currentRepresentations.audio).to.equal(
    ...
    describe("getVideoLoadedTime", () => {
      ...
      it("should return the time of the current loaded time", async function () {
        ...
        player.loadVideo({ url: manifestInfos.url, transport, autoPlay: false });
        await waitForLoadedStateAfterLoadVideo(player);
        await sleep(500);

        const initialBufferGap = player.getVideoBufferGap();
        expect(player.getPosition()).to.be.closeTo(minimumPosition, 0.1);
        expect(player.getVideoLoadedTime()).to.be.closeTo(initialBufferGap, 0.1);
        ...
        xhrMock.unlock();
        await sleep(300);
        expect(player.getVideoLoadedTime()).to.be.closeTo(initialBufferGap + 5, 10);
    ...
    describe("getVideoPlayedTime", () => {
      ...
      it("should return the difference between the start of the current range and the current time", async function () {
        ...
        expect(player.getVideoPlayedTime()).to.be.closeTo(initialLoadedTime + 5, 1);
      });
    });
    ...
    describe("getVideoBitrate", () => {
      it("should give a value once loaded", async () => {
        ...
        expect(player.getVideoBitrate()).to.equal(undefined);
        await waitForLoadedStateAfterLoadVideo(player);
        expect(player.getVideoBitrate()).to.be.oneOf(player.getAvailableVideoBitrates());
      });
    });
    ...
    describe("getAudioBitrate", () => {
      it("should give a value once loaded", async () => {
        ...
        expect(player.getAudioBitrate()).to.equal(undefined);
        await waitForLoadedStateAfterLoadVideo(player);
        expect(player.getAudioBitrate()).to.be.oneOf(player.getAvailableAudioBitrates());
      });
    });
    ...
    describe("setVideoBitrate", () => {
      ...
      it("should set the maximum bitrate if set to Infinity", async () => {
        ...
        if (videoBitrates.length) {
          expect(player.getVideoBitrate()).to.equal(
            videoBitrates[videoBitrates.length - 1],
          );
        } else {
          expect(player.getVideoBitrate()).to.equal(undefined);
        }
    ...
    describe("setAudioBitrate", () => {
      ...
      it("should set the maximum bitrate if set to Infinity", async () => {
        ...
        if (audioBitrates.length) {
          expect(player.getAudioBitrate()).to.equal(
            audioBitrates[audioBitrates.length - 1],
          );
        } else {
          expect(player.getAudioBitrate()).to.equal(undefined);
        }
