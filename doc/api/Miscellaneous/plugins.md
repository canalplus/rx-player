...
A `segmentLoader` allows to define a custom audio/video segment loader (it might in the
...
1. **segmentInfo** (`object`): An Object giving information about the wanted segments.
   ...
   - _trackType_ (`string`): The concerned type of track. Can be `"video"`, `"audio"`,
     `"text"` (for subtitles). Other values might be added in future versions, so
     applications should not assume that only those values can be encountered.
...
2. **callbacks**: An object containing multiple callbacks to allow this `segmentLoader` to
   ...
   - **resolve**: To call after the segment is loaded.

     When called, it should be given an object with the following properties:
     - _data_ (`ArrayBuffer`|`Uint8Array`|`null`) - the segment data which has not yet
       been communicated through the `data` callback. `null` can be set when all segment
       data has already been communicated.

       If not relying on the `data` callback (it is mostly useful for DASH Low-Latency
       contents), this property should contain the complete segment data.
   ...
   - **data**: Optional callback that can be called when a new subpart of the segment data
     has been loaded. This callback can be called multiple times, with consecutive
     subparts of the whole segment, before calling `resolve` once the request is complete.

     When called, it should be given an object with a `data` property containing an
     `ArrayBuffer` or `Uint8Array` corresponding to the newly loaded subpart.

     Some DASH Low-Latency contents rely on decodable subsegments. Here calling `data` as
     soon as possible instead of waiting for the full segment to call `resolve` will make
     the RxPlayer able to decode the media sooner.

     In most other cases however, calling `resolve` directly with the whole segment
     instead of consecutive `data` calls will have the same effect.
   ...
   - **fallback**: Callback to call if you want to call our default implementation instead
     for loading this segment.

     An optional object can be given with a `headers` property. `headers` should be an
     object describing HTTP headers that the default segment loader should add to its
     requests. Those headers have precedence over headers generated by the RxPlayer, such
     as `Range` and CMCD headers. Header names are compared without taking their casing
     into account.

     Overriding an RxPlayer-generated header may affect the loading behavior. For example,
     an initialization segment may necessitate multiple requests with different `Range`
     headers.

     `fallback` is an alternative to loading the segment through the custom loader. It
     should not be called after `data` or `resolve` has already been called.
...
2. **callbacks**: An object containing multiple callbacks to allow this `manifestLoader`
   ...
   - **fallback**: Callback to call if you want to call our default implementation instead
     ...
     An optional object can be given with a `headers` property. `headers` should be an
     object describing HTTP headers that the default Manifest loader should add to its
     request. Those headers have precedence over headers generated by the RxPlayer, such
     as CMCD headers. Header names are compared without taking their casing into account.
