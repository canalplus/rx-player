...
1. it automatically removes from the current media buffer the data linked to the
...
you will also need the API documented in the next part.

### `onKeyInternalError`, `onKeyOutputRestricted` and `onKeyExpiration`

Where `fallbackOnLastTry` really is about the failure of a license request, those options
...
reloading the same content later.
