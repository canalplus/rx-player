...
function isMediaCapabilitiesAPIAvailable() {
    return new Promise(function (resolve) {
        if (!("mediaCapabilities" in navigator)) {
            throw new Error("MediaCapabilitiesProber >>> API_CALL: " + "MediaCapabilities API not available");
        }
        ...
        if (!("decodingInfo" in navigator.mediaCapabilities)) {
            ...
            throw new Error("MediaCapabilitiesProber >>> API_CALL: " + "Decoding Info not available");
        }
        resolve();
    });
}
...
function probeDecodingInfos(config) {
    ...
    return isMediaCapabilitiesAPIAvailable().then(function () {
        var hasVideoConfig = config.type !== undefined &&
        ...
        var hasAudioConfig = config.type !== undefined &&
            config.type.length > 0 &&
        ...
        return navigator.mediaCapabilities
            ...
            return [result.supported ? types_1.ProberStatus.Supported : types_1.ProberStatus.NotSupported];
        })
            .catch(function () {
