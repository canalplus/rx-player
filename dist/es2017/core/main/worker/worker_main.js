...
function loadOrReloadPreparedContent(val, contentPreparer, playbackObservationRef, parentCancelSignal) {
    ...
    const { contentId, cmcdDataBuilder, manifest, mediaSource, representationEstimator, segmentSinksStore, segmentFetcherCreator, } = preparedContent;
    const { canFilterProtectionData, failOnEncryptedAfterClear, drmSystemId, enableFastSwitching, initialTime, onCodecSwitch, } = val;
    ...
    StreamOrchestrator({ initialPeriod: manifest.periods[0], manifest }, playbackObserver, representationEstimator, segmentSinksStore, segmentFetcherCreator, {
        wantedBufferAhead,
        maxVideoBufferSize,
        maxBufferAhead,
        maxBufferBehind,
