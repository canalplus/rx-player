...
function testMediaElement(x) {
    assertCompatibleIMediaElement(x);
}
function assertCompatibleIMediaElement(_x) {
    // Noop
}
// @ts-expect-error unused function, just used for compile-time typechecking
function testMediaSource(x) {
    assertCompatibleIMediaSource(x);
}
function assertCompatibleIMediaSource(_x) {
    // Noop
}
// @ts-expect-error unused function, just used for compile-time typechecking
function testSourceBuffer(x) {
    assertCompatibleISourceBuffer(x);
}
function assertCompatibleISourceBuffer(_x) {
    // Noop
}
// @ts-expect-error unused function, just used for compile-time typechecking
function testSourceBufferList(x) {
    assertCompatibleISourceBufferList(x);
}
function assertCompatibleISourceBufferList(_x) {
    // Noop
}
/* eslint-disable */
/** MediaSource implementation, including vendored implementations. */
const gs = globalScope;
const MediaSource_ = gs === undefined
    ? undefined
